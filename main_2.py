...
async def wait_for_safe_entry(entry: SafeEntry):
...
def main():
    ...
    if not buy_indices.empty:
        ...
        confirmed = asyncio.run(wait_for_safe_entry(safe_entry))

        if confirmed:
            trader.long("BTCUSDT", 1, 4)
            trailing_engine.start()
            print("✅ LONG entered after SAFE ENTRY confirmation")
            print("The Bullish sound")
            playsound(r'sounds/Bullish.wav')
        else:
            print("❌ LONG skipped — Safe Entry failed or timed out")
    ...
    if not sell_indices.empty:
        ...
        if confirmed:
            ...
            trailing_engine.start()
            print("✅ SHORT entered after SAFE ENTRY confirmation")
            print("The Bearish sound")
            playsound(r'./sounds/Bearish.wav')
        else:
            print("❌ SHORT skipped — Safe Entry failed or timed out")
