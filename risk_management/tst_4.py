    ...
    for p in positions:
        ...
        if amt != 0:
            ...
            return {
                ...
                "margin": margin,
                # SAFE: leverage is optional in Binance response
...
def calc_pnl_and_roi(pos, price):
    ...
    roi = pnl / margin if margin > 1e-8 else 0.0
    return pnl, roi
...
async def on_price_change(price):
    ...
    # SAFE Locked ROI calculation
    locked_roi = (
        (locked_pnl / pos["margin"]) * 100
        if pos["margin"] > 1e-8
        else 0.0
    )
