...
class Resample:
    ...
    def create_minute_data(self, s_id, symbol):
        for minute in self.minute_data:
            ...
            # # Ensure 'CloseTime' is part of the DataFrame and convert it to datetime
            # df_ = self.data.rename_axis('CloseTime_index')
            # # df_['CloseTime'] = df_.index.astype('int64') // 10**6
            # df_['CloseTime'] = df_.index.astype(
            #     'int64') // 10 ** 6  # Convert index (datetime) to Unix timestamp (milliseconds)
            #
            # rd = ResampleData(symbol)
            # asset_data = rd.resample_to_minute(df_, minute)
            #
            ...
            # asset_data.insert(0, 'symbol_id', symbol_id)
            #
            # print(f"Asset Data before enter the Database: :")
            # print(asset_data.head())
            # # print(input("Asset Data is going to enter the Database:"))
            #
            # # Store the resampled data in the SQLite database
            # with self.connection as con:
            #     con.execute(f'''CREATE TABLE IF NOT EXISTS asset_{minute}
            ...
            # asset_data.to_sql(name=f'asset_{minute}', con=self.connection, if_exists='append', index=False)
            #
            # print(input(f"Asset Data After Resampling : "))
            #
            #
            #
            # #################################
            ...
            #
            # print(input(f"Candle Data After Resampling : "))
            #
            # ########################
            # # Storing on rsi table #
            # ########################
