...
def main():
    ...
    plt.plot(all_Sum_data_frames[0]['Close_Price_1m'], label='Close Price')

    buy_indices = final_df.index[final_df['Total_Sum'] >= total_sum]
    plt.scatter(buy_indices, final_df['Close_Price_1m'][final_df['Total_Sum'] >= total_sum],
                marker='^', s=marker_sizes[final_df['Total_Sum'] >= total_sum], color='green',
                zorder=3)
    # Add text labels for sum values
    for index in buy_indices:
        plt.text(index, final_df['Close_Price_1m'][index], f'{(final_df["Total_Sum"])[index]}',
                 ha='center', va='bottom', fontsize=8)

    sell_indices = final_df.index[final_df['Total_Sum'] <= -total_sum]
    plt.scatter(sell_indices, final_df['Close_Price_1m'][final_df['Total_Sum'] <= -total_sum],
